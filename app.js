...
const costGroup03 = ["Archival Textured (310 gsm)", "Archival Photo Gloss Baryta (320 gsm)", "Archival Canvas (450 gsm)"];
...
// const textNode = document.createTextNode(`${estimate}`);
//#endregion	
...
const displayOutput = () => {
  ...
  estimate = `${inputHeight}x${inputWidth}${radioChoice} on ${substrate}: ${currencyFormat(printCost)}`;
  if (errorNote === "no errors") {
    addItem();
  } else {
  document.querySelector("#errorNote").innerHTML = `<p style="color:red">${errorNote}</p>`
  }
};
...
const addItem = (event) => {
  ...
  delButton.setAttribute("title", "Remove this item from the list");
...
const logBillValues = () => {
  console.log(`BILLING: billArea:${billArea}, cpi: ${cpi}, printCost: ${currencyFormat(printCost)}, Total: ${total}`);
};
